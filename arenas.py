...
class RatArena(composer.Arena):
    ...
    def _build(
            self, hfield, params, process_objects=False,
            pedestal_center=None,
            pedestal_radius=None,
            pedestal_height=None,
            arena_diameter=None, size=(8, 8), name='terrain'):
    ...
    def _process_objects(self):
        ...
        self.pedestal_center, self.pedestal_radius, self.pedestal_height = \
    ...
    def _get_arena_params(self):
        arena_diameter = \
            self.params['_ARENA_DIAMETER'] * self.params['scale_factor']
        res = self._ground_asset.nrow
        hfield_size = self._ground_asset.size[0]
        arena_px_size = \
            int(np.floor(res * (arena_diameter / (hfield_size * 2))))
        return arena_diameter, arena_px_size

    def _preprocess_hfield(self, hfield, sigma=2.5):
        hfield = cv2.resize(hfield, (self.arena_px_size, self.arena_px_size),
