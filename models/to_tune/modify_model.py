...
def scale_arms_and_legs(model, joint_pairs, bone_lengths):
    ...
    env = suite.load(domain_name="rat", task_name="stand")
    model_lengths = {k: get_bone_distance(env.physics, jp) * MM_TO_METER
    ...
    model_name_pairs = {
        ...
        'femur': [
            'knee', 'upper_leg_L0_collision',
            'upper_leg_R0_collision', 'lower_leg'],
        'tibia': ['ankle', 'foot'],
        'metatarsal': ['toe'],
        ...
        for g in model.find_all('geom'):
            if any(part in g.name for part in model_id):
                if bone == 'radius' and any(
                    part in g.name for part in [
                        'hand_L_collision', 'hand_R_collision']):
                    continue
...
if __name__ == "__main__":
    ...
    real_ratios = {name: val / np.sum(masses[0, :])
